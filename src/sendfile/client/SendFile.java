...
public class SendFile extends javax.swing.JFrame {
        ...
        public void run() {
            try {
                while(!Thread.currentThread().isInterrupted()){
                    String data = dis.readUTF();  // Baca konten data yang diterima dari Server
                    st = new StringTokenizer(data);
                    String cmd = st.nextToken();  // Dapatkan huruf pertama dari data
                    switch(cmd){
                        case "CMD_RECEIVE_FILE_ERROR":  // Format peasn: CMD_RECEIVE_FILE_ERROR [Message]
                            String msg = "";
    ...
    private void initComponents() {
        ...
        btnBrowse.setBackground(new java.awt.Color(139, 234, 185));
        ...
        btnSendFile.setBackground(new java.awt.Color(139, 234, 185));
        btnSendFile.setText("Kirim File");
    ...
    private void btnSendFileActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSendFileActionPerformed
        ...
        if((sendTo.length() > 0) && (file.length() > 0)){
            try {
                ...
                dos.writeUTF(format);
                System.out.println(format);
                updateBtn("Mengirim tututuuu...");
                btnSendFile.setEnabled(false);
            } catch (IOException e) {
                System.out.println(e.getMessage());
            }
        }else{
            JOptionPane.showMessageDialog(this, "Jangan biarkan kosong.!","Error", JOptionPane.ERROR_MESSAGE);
        }
    ...
    /*   Metode ini akan menonaktifkan/mengaktifkan semua GUI   */
